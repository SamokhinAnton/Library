...
{
    public class BorrowAction : IBookAction
    {
        ...
        public async Task Click(DataGridView dgv, DataGridViewCellEventArgs e, int bookId)
        {
            ...
            {
                MessageBox.Show("The book already borrowed");
            }
            else
            {
                await _repository.BorrowBookAsync(bookId);
                dgv.Rows[e.RowIndex].Cells["IsBorrowed"].Value = true;
            }
        }
    }
}
